...
const CreateRecord = ({ onRecordCreated }) => {
  ...
  const handleCreate = async () => {
    try {
      const response = await axios.post('http://localhost:2999/api/records', formData);
  ...
  return (
    <div>
      <Button variant="contained" onClick={handleOpenModal}>Criar</Button>
      <Modal open={modalOpen} onClose={handleCloseModal}>
        <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'background.paper', boxShadow: 24, p: 4, minWidth: 300 }}>
          <Typography variant="h5" gutterBottom>Criar Novo Registro</Typography>
          ...
          <Button variant="contained" onClick={handleCreate}>Confirmar</Button>
